...
description: 制作与 Claude / Anthropic 风格一致的 HTML 网页演示文稿(web deck / 网页 PPT)。当用户想做一套新的 HTML 幻灯片、复刻现有那套米白暖色风格的排版·动效、或为任意主题(技术分享、产品发布、项目汇报、培训讲座、学术报告等)生成网页演示时使用。自带完整可复用的 CSS/JS 引擎、组件库与骨架模板,无需从零搭建。
---

# claude-flavored-html · Claude 风格网页演示生成器

一套纯 **HTML / CSS / JS** 的演示框架(不是 PowerPoint)。特点:
- **暖色主题** `claude-warm`:米白纸感 + 陶土橙 + Fraunces 衬线。
- **设计令牌驱动**:颜色/圆角/阴影/字体全是 CSS 变量。
- **零依赖引擎** `runtime.js` 自动接管:`←/→/空格`翻页、`#/N`定位、`S`演讲者视图(逐字稿+计时器)、`O`总览、`F`全屏、进度条、**每次翻到某页自动重放该页入场动画**。
- 内置组件:渐变标题、圆角卡片、左边框 callout、胶囊 chip、数据表、状态追踪表、自绘 SVG 图、代码块;复合版式页:对比(VS)、竖向时间线、横向流程链、目录 agenda。

## 何时使用
- 用户要"做一个 PPT / 演示 / slides / 讲稿",且接受**网页形式**。
- 用户要"复刻 / 沿用 / 套用这种风格"为**新主题**做演示。
- 用户给一份大纲、提纲或素材,想要成品幻灯片。

## 资产清单(全部在本 skill 目录内)
- **`deck-template/`** —— 起点,**整个复制过去**当新 deck:
  - `assets/`(`base.css` 令牌+幻灯片系统、`animations.css` 动效库、`fonts.css` 网络字体、`runtime.js` 引擎、`themes/claude-warm.css` 主题)—— **原样复用,一字不改**。
  - `style.css` —— 组件库 —— **原样复用,一字不改**。
  - `index.html` —— 可直接打开的**骨架模板**,含封面/内容页/分隔页/追踪表页/要点总结页/数据展示页/目录/对比/时间线/流程/结尾共 12 张示例页,展示所有版式与组件,照它增删即可。
- **`reference/components.md`** —— 完整组件词汇表 + SVG 绘制约定 + 每页骨架。**开始写内容前必读**。

## 工作流程
1. **确认输入**:主题、受众、大纲要点。缺则先问。
...
5. **浏览器自检**:翻页正常、按 `O` 总览、每页内容**一屏内不溢出不滚动**。无法目测时,用 `index.html?audit` 打开——引擎会逐页检测溢出,在左上角给出徽标并在控制台列出溢出页。

## 不可违背的硬约束
1. 文档头照抄模板:`<html ... data-theme="claude-warm">`;`<head>` 内按 **fonts → base → animations → theme-link(`id="theme-link"`) → style.css** 顺序引入;`<body class="tpl">`;外层 `<div class="deck">`。
...
6. `<script src="assets/runtime.js"></script>` 放在 `</body>` 前。**不要写任何 JS**——翻页/动画重放全由引擎接管。
7. 颜色/圆角/阴影/字体**只用 `var(--…)` 令牌**,禁止硬编码色值(个别强调可 `color:var(--accent)`)。
8. 设计分辨率 **1920×1080(16:9)**;留白充足,确保不溢出。引擎会把这块 1920×1080 舞台**自动等比缩放**适配任意屏幕(留黑边),所以你只管按 1920×1080 排版,换屏不会错位。
...
- [ ] 无硬编码颜色,只用 `var(--…)` 令牌。
- [ ] 每页一屏内不溢出(可用 `?audit` 逐页核查);`O` 总览缩略图排版正常。
