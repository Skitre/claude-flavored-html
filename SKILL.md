...
- 内置组件:渐变标题、圆角卡片、左边框 callout、胶囊 chip、数据表、算法迭代表、自绘 SVG 树/图、代码块;复合版式页:对比(VS)、竖向时间线、横向流程链、目录 agenda。
...
5. **浏览器自检**:翻页正常、按 `T` 两套配色都好看、按 `O` 总览、每页内容**一屏内不溢出不滚动**。无法目测时,用 `index.html?audit` 打开——引擎会逐页检测溢出,在左上角给出徽标并在控制台列出溢出页。
