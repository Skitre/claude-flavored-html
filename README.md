...
把"复刻那套米白暖色 HTML 演示文稿"的能力封装成了一个 Claude Code Skill。
装好后,直接对 Claude 说「用 claude-flavored-html 给『XXX 主题』做一套演示」即可。
...
└── deck-template/            ← 起点:整个复制过去当新 deck
    ├── index.html            ← 含 12 种版式的空骨架,照它增删
    ├── style.css             ← 组件库(原样复用)
    └── assets/               ← 引擎/令牌/字体/动效/主题(原样复用)
        ├── base.css  animations.css  fonts.css  runtime.js
        └── themes/  claude-warm.css
```
...
- 或显式调用:`/claude-flavored-html` 后接你的主题、大纲、素材。
...
快捷键:`← →`翻页、`S`演讲者视图(逐字稿+计时器)、`O`总览、`F`全屏。
